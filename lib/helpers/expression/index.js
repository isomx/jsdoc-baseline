 ...
 *
 * In order to implement this fix, the following template file
 * was modified:
 ...
 * inherited class property who for some reason never
...
function _handleParsedTypeMod(type) {
    if (type.parsedType) {
         ...
         * been parsed. But if it doesn't have the
         * parsedType property defined (enumerable),
         * it means it is the result of an inherited
         * property, and this is my bug fix for it
         * showing those properties as 'unknown'
         */
        const names = type.names;
        if (names.length > 1) {
            /*
             * This should never happen. Because the only
             * bug that I'm aware of so far is inherited
             * properties in a Class, in which case I don't
             * see what scenario you would have a Class property
             * that has more than 1 type. But this is how
             * it should be parsed.
             *
             * Note this only naively assumes it is a union
             * type of { type: "NameExpression", name: "<the name>" }.
             * This won't work for more complex types like "TypeApplication".
             * But again, this should never happen anyway
             */
            let elements = [];
            for(let name of names) {
                elements.push({ type: "NameExpression", name });
            }
