...
/*
 * This is my modification to deal with inherited properties
 * from a child class appearing as "unknown" on the parent class.
 * And also inherited method properties & return types not showing up.
 *
 * From what I can gather, it is not going through the process
 * of saving the parsedType result as a defined property on
 * the "type" (which then calls Object.freeze). So
 * type.parsedType should be a defined property on the type,
 * and it is enumerable so it won't show up when logging it to console.
 * This occurs in the catharsis package. The bug only affects
 * inherited class properties/methods.
 *
 * In order to implement this fix, the following template files
 * were modified:
 *
 * views/partials/signature.hbs
 * line 33: ... {~describeType type.parsedType~} was changed to:
 *      {~describeType type~}
 *
 * views/partials/details-table-row.hbs
 * line 24: ... {#all type type.parsedType} changed to: {#all type}
 * line 25: ... {describeType type.parsedType 'extended'} was changed to:
 *      {describeType type 'extended'}
 *
 * views/atoms/returns.hbs
 * line 21: ... {~#all type type.parsedType~} changed to: {~#all type~}
 * line 22: ... {describeType type.parsedType} changed to: {describeType type}
 *
 *
 * NOTE: You obviously must also call this function in
 * describeType() (located below in this file's exports).
 * See my notes in that method
 *
 * So we are given the full type instead, and
 * this method deals with either calling type.parsedType,
 * or manually building the parsedType if it is for an
 * inherited class property/method/return who for some reason never
 * gets the parsedType defined on it.
 *
 * Other template locations call describeType(),
 * so the type argument may already be the parsedType.
 *
 * Note: This is really a pretty amateur hack. I have
 * no idea what the root cause of the problem is. But
 * this seems to be a stable fix. It doesn't appear
 * that disabling caching on the catharsis call to parseType
 * helps, but that was just a wild guess anyway.
 *
 */
const __catharsisOpts = { jsdoc: true };
function _handleParsedTypeMod(type) {
    let parsedType = type.parsedType;
    if (parsedType) {
        return parsedType;
    } else if (type.names) {
        /*
         * This appears to be a safe check, because as
         * far as I can tell there is no 'type' that
         * uses the `names` key, except when it hasn't
         * been parsed.
         */
        ...
        if (names.length > 1) {
            ...
            for(let name of names) {
                parsedType = catharsis.parse(name, __catharsisOpts);
                elements.push(parsedType);
            }
            return { type: 'TypeUnion', elements };
        } else {
            return catharsis.parse(names[0], __catharsisOpts);
        }
    } else {
        /*
         * It was already a parsedType, since other
         * template locations call describeType() besides
         * what I modified
         */
        return type;
    }
...
// template is an instance of the ../../template.js class
module.exports = template => {
    ...
    init();
  /**
   * An Object whose keys are the absolute
   * path of the source file, and the values
   * are a boolean indicating that it should
   * be "private" (i.e., not published). This
   * **does not** affect documentation, only
   * the `Source Code:` link.
   * @type {Object.<string, boolean>}
   * @ignore
   */
  const privateSrcFilesMap = template.privateSrcFilesMap;
  const linkFormatter = template.linkFormatter;
    ...
    return {
        ...
        describeType(parsedType, format, property) {
            ...
            let description;
            /**
             * This is my mod to deal with inherited class properties
             * appearing "unknown". See the function for further notes
             */
            parsedType = _handleParsedTypeMod(parsedType);
        ...
        link(item, linkText, linkClass, fragmentId) {
            ...
            if (typeof item !== 'string') {
                ...
                linkFormatter.checkForBrokenLinks(item);
                if (
                    linkFormatter.longnameToUrl
                    && item.indexOf('@link') < 0
                    && !/\s/g.test(item)
                    && !linkFormatter.longnameToUrl[item]
                ) {
                    linkFormatter.addToBrokenLinks(item);
                }
            }
         ...
         */
        resolveLinks(text) {
            linkFormatter.checkForBrokenLinks(text);
        ...
        url(longname) {
            if ({}.hasOwnProperty.call(templateHelper.longnameToUrl, longname)) {
                ...
                linkFormatter.addToBrokenLinks(longname);
            }
        ...
        // my addition, switching to highlight js
        highlightCode(code) {
            return hljs.highlight(code, { language: 'js' }).value;
        },

        // my addition to check whether source code of
        // a file should be published
        isSrcFilePrivate(path, filename) {
            return !!privateSrcFilesMap[join(path, filename)];
        }
    };
};
