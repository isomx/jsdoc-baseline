...
module.exports = class Template {
    constructor(conf, opts) {
       ...
       * @ignore
       */
        this.privateSrcFilesMap = opts.privateSrcFilesMap;
        this.linkFormatter = opts.linkFormatter;
    }
    ...
    render(viewName, data, options) {
        ...
        data.intl = i18nData;
        // My additions to add additional data
        const config = this.config;
        data.themeCookie = config.themeCookie;
        let htmlTagClassName = config.htmlTagClassName;
        if (config.embedded) {
            htmlTagClassName = 'rmd-embed';
            if (config.excludeNav) {
                htmlTagClassName += ' rmd-embed--no-nav';
            } else {
                htmlTagClassName += ' rmd-embed--with-nav';
            }
        }
        if (config.defaultTheme) {
            if (!htmlTagClassName) {
                htmlTagClassName = `${config.defaultTheme}-theme`;
            } else {
                htmlTagClassName += ` ${config.defaultTheme}-theme`;
            }
        }
        data.htmlTagClassName = htmlTagClassName || '';
        data.excludeNav = config.excludeNav;
        data.defaultTheme = config.defaultTheme;
        data.bodyContentOnly = config.bodyContentOnly || false;
    ...
    translate(key, opts) {
        ...
        if (!{}.hasOwnProperty.call(this.l10nFormatters, key)) {
            formatString = _.get(this.l10nData, key);
