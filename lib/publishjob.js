...
module.exports = class PublishJob {
    constructor(template, options) {
        ...
        this.linkFormatter.setLongnameToUrl(helper.longnameToUrl);
    ...
    generate(viewName, data, url) {
        ...
        if (viewName === 'symbol') {
            ...
            output = output
                ...
                .replace(/<em>perm_rec_delete<\/em>/g, '_perm_rec_delete_')
                .replace(/<em>perm_rec_read<\/em>/g, '_perm_rec_read_')
                .replace(/<em>perm_rec_update<\/em>/g, '_perm_rec_update_')
        }
        try {
    ...
    generateTocData(options) {
        ...
        class TocItem {
            constructor(item, children) {
                ...
                linkFormatter.checkForBrokenNavLink(item.longname);
    ...
    generateRawDataOutput(docletHelper) {
        ...
        const excludedDocletsIgnore = outConfig.excludedDocletsIgnoreList;
        const linkFormatter = this.linkFormatter;
        ...
        for (let key in _all) {
            ...
            if (!validUrls[stripHash(_longnameToUrl[key])]) {
                if (excludedDocletsIgnore) {
                    let longname = doclet.longname;
                    let curr = linkFormatter.stripMember(longname), found = false;
                    while(curr !== longname) {
                        if (excludedDocletsIgnore.indexOf(curr) > -1) {
                            found = true;
                            break;
                        }
                        longname = curr;
                        curr = linkFormatter.stripMember(longname);
                    }
                    if (found) {
                        continue;
                    }
        ...
        if (outConfig.brokenLinks) {
            __writeFile(
                path.join(outDir, 'brokenLinks.json'),
                toString(this.linkFormatter.getBrokenLinksResult() || []),
                encoding
            );
        }
        if (outConfig.brokenNavLinks) {
            __writeFile(
                path.join(outDir, 'brokenNavLinks.json'),
                toString(this.linkFormatter.getBrokenNavLinksResult() || []),
                encoding
            );
        }
