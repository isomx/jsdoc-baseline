...
module.exports = class PublishJob {
    constructor(template, options, linkFormatter, privateSrcFilesMap) {
        this.linkFormatter = linkFormatter;
        this.privateSrcFilesMap = privateSrcFilesMap;
        /*
        ...
        this.indexUrl = linkFormatter.addBase(helper.getUniqueFilename('index'));
        helper.registerLink('index', this.indexUrl);
        this.globalUrl = linkFormatter.addBase(helper.getUniqueFilename('global'));
    ...
    generateRawDataOutput(docletHelper) {
        ...
        const outConfig = config.dataOutput;
        let outDir = outConfig && outConfig.dir;
        if (!outDir) {
            return this;
        }
        ...
        if (outConfig.navTree) {
            __writeFile(
                path.join(outDir, 'navTree.js'),
                `module.exports = ${toString(navTree)};`,
                encoding
            );
        }
        if (outConfig.navData) {
            __writeFile(
                path.join(outDir, 'navData.js'),
                `module.exports = ${toString(flatTree)}`,
                encoding
            );
        }
        if (outConfig.navUrlBaseToItemId) {
            __writeFile(
                path.join(outDir, 'navUrlBaseToItemId.js'),
                `module.exports = ${toString(urlBaseToItemId)}`,
                encoding
            );
        }
        if (outConfig.docletsTree) {
            __writeFile(
                path.join(outDir, 'docletsTree.js'),
                `module.exports = ${toString(docletHelper.allLongnamesTree)};`,
                encoding
            );
        }
        if (outConfig.doclets) {
            __writeFile(
                path.join(outDir, 'doclets.js'),
                `module.exports = ${toString(all)};`,
                encoding
            );
        }
        if (outConfig.longnameToUrl) {
            __writeFile(
                path.join(outDir, 'longnameToUrl.js'),
                `module.exports = ${toString(longnameToUrl)};`,
                encoding
            );
        }
        if (outConfig.longnameToSrcUrl) {
            __writeFile(
                path.join(outDir, 'longnameToSrcUrl.js'),
                `module.exports = ${toString(longnameToSrcUrl)};`,
                encoding
            );
        }
        if (outConfig.excludedDocletsWithHash) {
            __writeFile(
                path.join(outDir, 'excludedDocletsWithHash.js'),
                `module.exports = ${toString(excludedWithHash)};`,
                encoding
            );
        }
        if (outConfig.excludedDocletsWithoutHash) {
            __writeFile(
                path.join(outDir, 'excludedDocletsWithoutHash.js'),
                `module.exports = ${toString(excludedWithoutHash)};`,
                encoding
            );
        }
        return this;
